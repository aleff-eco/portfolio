...
export default function NotificationButton() {
  ...
  const [newCount] = useState(true); // Simula notificaciones entrantes continuamente
  ...
  return (
    <div className="fixed top-4 md:top-8 lg:top-12 right-4 md:right-8 lg:right-16 z-50">
      ...
      >
        ...
        <span className="absolute top-0 right-0 flex items-center justify-center h-6 w-6 text-sm font-bold"
          style={{
            color: newCount ? "rgba(255, 0, 0, 0.6)" : "transparent",}}>
          {repos.length}
        </span>
      </motion.button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute top-full mt-3 right-0 w-96 bg-white dark:bg-gray-900 rounded-[20px] shadow-2xl overflow-hidden z-10"
